...
public class HomeworkSubjectWiseAdapter extends RecyclerView.Adapter<HomeworkSubjectWiseAdapter.ViewHolder> {
    private List<Homework> mAssignmentList;
    List<Integer> mSelectedPosition;


    public HomeworkSubjectWiseAdapter(List<Homework> mAssignmentList, ArrayList<Integer> selectedPosition) {
        this.mAssignmentList = mAssignmentList;
        this.mSelectedPosition = selectedPosition;
    }

    @Override
    public HomeworkSubjectWiseAdapter.ViewHolder onCreateViewHolder(ViewGroup parent,
                                                                    int viewType) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        if (!TextUtils.isEmpty(homeworkType) && !homeworkType.equalsIgnoreCase(ConstantUtil.SUBMITTED)) {
            ...
            String text = "Submitted on " + DateUtils.getFormatedDateFromDate(DateUtils.convertrIsoDate(homework.getSubmittedDate())).toUpperCase();
            holder.binding.textviewSubmittedDate.setText(text);
        } else {
            holder.binding.textviewDueOn.setVisibility(View.GONE);
            holder.binding.textviewSubmittedDate.setVisibility(View.GONE);
        }

        if (!TextUtils.isEmpty(homeworkType) && homeworkType.equalsIgnoreCase(ConstantUtil.OVERDUE) && mSelectedPosition.indexOf(position) != -1) {

            holder.binding.textviewHomeworkType.setVisibility(View.VISIBLE);
            holder.binding.textviewHomeworkType.setText(ConstantUtil.OVERDUE);
        } else if (!TextUtils.isEmpty(homeworkType) && homeworkType.equalsIgnoreCase(ConstantUtil.PENDING) && mSelectedPosition.indexOf(position) != -1) {
            holder.binding.textviewHomeworkType.setVisibility(View.VISIBLE);
            holder.binding.textviewHomeworkType.setText(ConstantUtil.PENDING);

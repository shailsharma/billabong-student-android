package in.securelearning.lil.android.mindspark.views.activity;
...
import in.securelearning.lil.android.mindspark.dataobjects.MindSparkQuestionContentData;
...
import in.securelearning.lil.android.mindspark.dataobjects.MindSparkQuestionTrial;
...
import in.securelearning.lil.android.mindspark.dataobjects.MindSparkQuestionUserAttempt;
import in.securelearning.lil.android.mindspark.model.MindSparkModel;
import in.securelearning.lil.android.mindspark.views.adapter.DropdownChoiceAdapter;
...
import static in.securelearning.lil.android.mindspark.MindSparkConstants.ACTION_START;
import static in.securelearning.lil.android.mindspark.MindSparkConstants.MODE_LEARN;
import static in.securelearning.lil.android.mindspark.MindSparkConstants.MS_LANGUAGE_CODE;
import static in.securelearning.lil.android.mindspark.MindSparkConstants.MS_VERSION_ID_SUFFIX;
...
public class MindSparkPlayerActivity extends AppCompatActivity {
    ...
    private void fetchQuestion() {
        if (GeneralUtils.isNetworkAvailable(getBaseContext())) {
            final Dialog dialog = mFlavorHomeModel.loadingDialog(MindSparkPlayerActivity.this, getString(R.string.messagePleaseWait));
            ...
            mMindSparkModel.getMindSparkQuestion(mindSparkQuestionRequest)
                        ...
                        public void accept(MindSparkQuestionParent mindSparkQuestionParent) throws Exception {
                            ...
                            if (mindSparkQuestionParent.getRedirectionData() != null && mindSparkQuestionParent.getRedirectionData().isEndTopicFlag()) {
                                ...
                                if (mindSparkQuestionContentData.getQuestions().get(0).getQuestionResponse() != null) {
                                    Question question = mMindSparkModel.convertMSQuestionToLILQuestion(mindSparkQuestionContentData.getContentSeqNum(), mindSparkQuestionContentData.getContentId(), mindSparkQuestionContentData.getQuestions().get(0));
    ...
    private void submitAndFetchNewQuestion(final Question question) {
        if (GeneralUtils.isNetworkAvailable(getBaseContext())) {
            final Dialog dialog = mFlavorHomeModel.loadingDialog(MindSparkPlayerActivity.this, getString(R.string.messageGettingNextQuestion));
            mMindSparkModel.submitAndFetchNewQuestion(prepareSubmitResponse(question))
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(new Consumer<MindSparkQuestionParent>() {
                        @Override
...
//
    ...
    private void setIFrameInWebView(String questionText) {
        int iFrameCount = mMindSparkModel.checkWordOccurrence(questionText, "<iframe");
        if (iFrameCount > 0) {
            ArrayList<String> iFrameList = mMindSparkModel.getIFrameListFromString(iFrameCount, questionText);
    ...
    private void showExplanation(final Question question) {
        if ((question.getChoiceConfiguration().getQuestionExplanation() != null && !TextUtils.isEmpty(question.getChoiceConfiguration().getQuestionExplanation().getExplanationText())) || !TextUtils.isEmpty(question.getExplanation())) {
            ...
            if (!TextUtils.isEmpty(explanationText)) {
                ...
                mBinding.textViewExplanationText.setHtml(MindSparkConstants.decodeBase64String(explanationText), htmlHttpImageGetter);
            } else {
                mBinding.layoutExplanation.setVisibility(View.GONE);
            }

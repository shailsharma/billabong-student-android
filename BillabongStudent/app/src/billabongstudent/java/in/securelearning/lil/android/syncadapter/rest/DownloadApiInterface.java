...
import in.securelearning.lil.android.syncadapter.dataobjects.RecommendedApiObject;
...
public interface DownloadApiInterface {
    ...
    //@POST("http://192.168.0.136:8082/lessonPlanConfiguration/getPracticeQuestions")
    ...
    @POST("UserChallanges/other/{status}")
    ...
    /*Api to fetch vocational topic (for now logiqids)*/
    @POST("TPCurriculumMappings/getTPCurriculumMapping")
    Call<java.util.ArrayList<VocationalTopic>> fetchVocationalTopics(@Body VocationalTopicRequest topicRequest);

    /*Api to fetch help and faq data*/
    @GET("FAQs/all")
    Call<java.util.ArrayList<HelpAndFaqCategory>> fetchHelpAndFAQ();

    /*Api to fetch revision subjects with details*/
    @GET("PracticeResponses/get-revision-subject-list")
    Call<java.util.ArrayList<RevisionSubject>> fetchRevisionSubjects();

    /*Api to fetch list of questions for revision*/
    @POST("PracticeResponses/get-revision-questions")
    Call<RevisionResponse> fetchQuestionsAndSubmitRevision(@Body RevisionResponsePost revisionResponsePost);

    /*Api to fetch Geo-Gebra card detail list for Apply*/
    @POST("TPCurriculumMappings/fetchMappingsForType")
    Call<java.util.ArrayList<TPCurriculumResponse>> fetchGeoGebraCardDetail(@Body ThirdPartyMapping thirdPartyMapping);
}
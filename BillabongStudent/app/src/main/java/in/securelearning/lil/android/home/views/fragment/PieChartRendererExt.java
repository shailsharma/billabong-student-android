...
/**
 * Created by Prabodh Dhabaria on 14-04-2017.
 */

public class PieChartRendererExt extends DataRenderer {

    protected PieChartExt mChart;
    ...
    protected Paint mHolePaintInner;
    ...
    public PieChartRendererExt(PieChartExt chart, ChartAnimator animator,
        ...
        mHolePaintInner = new Paint(Paint.ANTI_ALIAS_FLAG);
        mHolePaintInner.setColor(Color.WHITE);
        mHolePaintInner.setAlpha(100);
        mHolePaintInner.setStyle(Paint.Style.FILL);
    ...
    public void drawData(Canvas c) {
        ...
        PieDataExt pieData = mChart.getData();

        for (IPieDataSet set : pieData.getDataSets()) {

            if (set.isVisible() && set.getEntryCount() > 0)
                drawDataSet(c, set, pieData.getInnerDataCount());
        }
    }

    private Path mPathBuffer = new Path();
    private Path mPathBufferInner = new Path();
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet, List<Float> innerDataCount) {
        ...
        for (int j = 0; j < entryCount; j++) {
//            final float radius2 = radius * ((j+1) * 30.f / 100.f);
            final float radius2 = radius * (innerDataCount.get(j) / 100.f);
            float sliceAngle = drawAngles[j];
            float innerRadius = radius2;

            final RectF circleBoxInner = new RectF();
            circleBoxInner.set(
...
//                    paintInner.setAlpha(50);
                    ...
                    mPathBuffer.reset();
                    mPathBufferInner.reset();

                    float arcStartPointX = 0.f, arcStartPointY = 0.f;
                    float arcStartPointXInner = 0.f, arcStartPointYInner = 0.f;

                    if (sweepAngleOuter % 360f == 0.f) {
                        // Android is doing "mod 360"
                        mPathBuffer.addCircle(center.x, center.y, radius, Path.Direction.CW);
                        mPathBufferInner.addCircle(center.x, center.y, innerRadius, Path.Direction.CW);
                    } else {
                        ...
                        arcStartPointXInner = center.x + innerRadius * (float) Math.cos(startAngleOuter * Utils.FDEG2RAD);
                        arcStartPointYInner = center.y + innerRadius * (float) Math.sin(startAngleOuter * Utils.FDEG2RAD);

                        mPathBuffer.moveTo(arcStartPointX, arcStartPointY);
                        mPathBufferInner.moveTo(arcStartPointXInner, arcStartPointYInner);
                    ...
                    mPathBufferInner.lineTo(
                            center.x,
                            center.y);
                    mPathBuffer.close();
                    mPathBufferInner.close();

                    mBitmapCanvas.drawPath(mPathBuffer, mRenderPaint);
                    mBitmapCanvas.drawPath(mPathBufferInner, paintInner);
                }
            }
    ...
    public void drawValues(Canvas c) {
        ...
        PieDataExt data = mChart.getData();

...
//        ArrayList<Resource> list = FileUtils.getResourceFileListFromInternalStorage(mContext);
    ...
    public LearnigMapUtils getLearningMap(String gradeId, String subjectId, String topicId, int skip, int limit) {
        LearnigMapUtils learningMapUtils = new LearnigMapUtils();
        ArrayList<LearningMap> learningMap = learningMapModel.getLearningMapList(gradeId, subjectId, topicId, "", skip, limit);
        HashMap<String, SubjectMap> subjects = new HashMap<>();
        ...
        if (learningMap == null || learningMap.size() <= 0) {
            return learningMapUtils;
        } else {
            for (int k = 0; k < learningMap.size(); k++) {
                SkillMap skillMap = new SkillMap();
                skillMap.setId(learningMap.get(k).getSkill().getId());
                skillMap.setTid(learningMap.get(k).getTopic().getId());
                skillMap.setSid(learningMap.get(k).getSubject().getId());
                skillMap.setName(learningMap.get(k).getSkill().getSkillName());
                skillMap.setSkillLevel(learningMap.get(k).getSkill().getSkillLevel());
                skillMap.setTotalObtained((learningMap.get(k).getSkill().getMarksObtained() * 100));
                skillMapList.add(skillMap);
                if (skillMap.getTotalObtained() >= 60) {
                    learningMapUtils.getSkill()[0] = learningMapUtils.getSkill()[0] + 1;
                } else if (skillMap.getTotalObtained() >= 36) {
                    learningMapUtils.getSkill()[1] = learningMapUtils.getSkill()[1] + 1;
                } else {
                    learningMapUtils.getSkill()[2] = learningMapUtils.getSkill()[2] + 1;
                }

                if (!subjects.containsKey(learningMap.get(k).getSubject().getId())) {
                    subjects.put(learningMap.get(k).getSubject().getId(), new SubjectMap(0, 0, learningMap.get(k).getSubject().getName(), learningMap.get(k).getSubject().getId()));
                }

                final SubjectMap subjectMap = subjects.get(learningMap.get(k).getSubject().getId());


                if (!subjectMap.getmTopicMap().containsKey(learningMap.get(k).getTopic().getId())) {
...
//                    score = (score * count) / (count + 1);
                    ...
                    subjectMap.getmTopicMap().put(learningMap.get(k).getTopic().getId(), new TopicMap(0, 0, learningMap.get(k).getTopic().getName(), learningMap.get(k).getTopic().getId(), learningMap.get(k).getSubject().getId()));

                }

                final TopicMap topicMap = subjectMap.getmTopicMap().get(learningMap.get(k).getTopic().getId());
                ...
                if (topicMap.isAddedOnce()) {
                    if (oldScore >= 60 && learningMapUtils.getTopic()[0] > 0) {
                        learningMapUtils.getTopic()[0] = learningMapUtils.getTopic()[0] - 1;
                    } else if (oldScore >= 36 && learningMapUtils.getTopic()[1] > 0) {
                        learningMapUtils.getTopic()[1] = learningMapUtils.getTopic()[1] - 1;
                    } else if (learningMapUtils.getTopic()[2] > 0) {
                        learningMapUtils.getTopic()[2] = learningMapUtils.getTopic()[2] - 1;
                    }
                ...
                if (subjectMap.isAddedOnce()) {
                    if (oldScoreSub >= 60 && learningMapUtils.getSubject()[0] > 0) {
                        learningMapUtils.getSubject()[0] = learningMapUtils.getSubject()[0] - 1;
                    } else if (oldScoreSub >= 36 && learningMapUtils.getSubject()[1] > 0) {
                        learningMapUtils.getSubject()[1] = learningMapUtils.getSubject()[1] - 1;
                    } else if (learningMapUtils.getSubject()[2] > 0) {
                        learningMapUtils.getSubject()[2] = learningMapUtils.getSubject()[2] - 1;
                    }
                }

                if (newScoreSub >= 60) {
                    learningMapUtils.getSubject()[0] = learningMapUtils.getSubject()[0] + 1;
                } else if (newScoreSub >= 36) {
                    learningMapUtils.getSubject()[1] = learningMapUtils.getSubject()[1] + 1;
                } else {
                    learningMapUtils.getSubject()[2] = learningMapUtils.getSubject()[2] + 1;
                }
            ...
            learningMapUtils.setSubjectArrayList(subjects);
            learningMapUtils.setSkillMapList(skillMapList);
            learningMapUtils.setTopicMapList(new ArrayList<>(topicMapList.values()));
            return learningMapUtils;
        }
    }
...
//        c.set(Calendar.MILLISECOND,0);
    ...
    public LearnigMapUtils getDefaultMapValue() {
        LearnigMapUtils learningMapUtils = new LearnigMapUtils();
        final UserProfile loggedInUser = mAppUserModel.getApplicationUser();
        String gradeId = "";
        if (loggedInUser.getGrade() != null) {
            gradeId = loggedInUser.getGrade().getId() != null ? loggedInUser.getGrade().getId() : "";
        }
        ArrayList<LearningMap> learningMap = learningMapModel.getLearningMapList(gradeId, "", "", "", 0, 0);
        HashMap<String, SubjectMap> subjects = new HashMap<>();
        HashMap<String, TopicMap> topicMapList = new HashMap<>();
        ArrayList<HomeModel.SkillMap> skillMapList = new ArrayList<>();
        if (learningMap == null || learningMap.size() <= 0) {
            ...
            for (int k = 0; k < learningMap.size(); k++) {
                ...
                skillMap.setName(learningMap.get(k).getSkill().getSkillName());
                skillMap.setSkillLevel(learningMap.get(k).getSkill().getSkillLevel());
                skillMap.setBoard(learningMap.get(k).getBoard());
                skillMap.setGrade(learningMap.get(k).getGrade());
                skillMap.setLanguage(learningMap.get(k).getLang());
                skillMap.setLearningLevel(learningMap.get(k).getLearningLevel());
                skillMap.setSubject(learningMap.get(k).getSubject());
                skillMap.setTopic(learningMap.get(k).getTopic());
                try {
                    int totalQuestionsAttempted = new Double(learningMap.get(k).getSkill().getTotalQuestions()).intValue();
...
//    }
    ...
    public LearnigMapUtils getDefaultLearningMapValue() {
        ArrayList<LearningMap> learningMap = getLearningMapList();
        LearnigMapUtils learningMapUtils = new LearnigMapUtils();
        int[] skillList = {0, 0, 0};
        int red = 0;
        int green = 0;
        ...
        for (int i = 0; i < learningMap.size(); i++) {

            if (!topics.containsKey(learningMap.get(i).getTopic().getId())) {
                topics.put(learningMap.get(i).getTopic().getId(), learningMap.get(i).getTopic());
            }

            if (!subjects.containsKey(learningMap.get(i).getSubject().getId())) {
                subjects.put(learningMap.get(i).getSubject().getId(), learningMap.get(i).getSubject());
            }

            if (!subjects.get(learningMap.get(i).getSubject().getId()).getTopics().contains(learningMap.get(i).getTopic())) {
                subjects.get(learningMap.get(i).getSubject().getId()).getTopics().add(learningMap.get(i).getTopic());
            }

            topics.get(learningMap.get(i).getTopic().getId()).getSkills().add(learningMap.get(i).getSkill());
            int index = subjects.get(learningMap.get(i).getSubject().getId()).getTopics().indexOf(learningMap.get(i).getTopic());
            subjects.get(learningMap.get(i).getSubject().getId()).getTopics().get(index).getSkills().add(learningMap.get(i).getSkill());

            double marksObtained = learningMap.get(i).getSkill().getMarksObtained();
            percentage = marksObtained * 100;
            if (percentage >= 60) {
                green++;
        ...
        skillList[1] = orange;
        skillList[2] = red;
        learningMapUtils.setSkill(skillList);

        ArrayList<Subject> subjectArrayList = new ArrayList<>();
        subjectArrayList.addAll(subjects.values());

        //topic hashmap  for each mTopic calculate score
        for (Subject sub : subjectArrayList) {
            ...
            for (Topic topic :
                ...
                for (Skill skill :
                        topic.getSkills()) {
                    topic.setMarksObtained(skill.getMarksObtained());
                    sumForTopic += (skill.getMarksObtained() * 100);
                }
                topic.setScore(sumForTopic / topic.getSkills().size());
                topics.get(topic.getId()).setScore(topic.getScore());

                percentage = topic.getScore();
                if (percentage >= 60) {
                    learningMapUtils.getTopic()[0] = learningMapUtils.getTopic()[0]++;
                } else if (percentage >= 35) {
                    learningMapUtils.getTopic()[1] = learningMapUtils.getTopic()[1]++;
                } else {
                    learningMapUtils.getTopic()[2] = learningMapUtils.getTopic()[2]++;
                }
                sunForSub += topic.getScore();
            }
            sub.setScore(sunForSub / sub.getTopics().size());
            subjects.get(sub.getId()).setScore(sub.getScore());
            percentage = sub.getScore();
            if (percentage >= 60) {
                learningMapUtils.getSubject()[0] = learningMapUtils.getSubject()[0]++;
            } else if (percentage >= 35) {
                learningMapUtils.getSubject()[1] = learningMapUtils.getSubject()[1]++;
            } else {
                learningMapUtils.getSubject()[2] = learningMapUtils.getSubject()[2]++;
            }
